...
market_buy = MarketOrderRequest(
    symbol=random_stonk[0], qty=1, side=OrderSide.BUY, time_in_force=TimeInForce.GTC
)

market_sell = MarketOrderRequest(
    symbol=random_stonk[0], qty=1, side=OrderSide.SELL, time_in_force=TimeInForce.GTC
)
...
if get_bear > get_bull:
    ...
    get_bear < get_bull and trading_client.get_asset(random_stonk[0]).shortable is True
):
    print("Inversing StockTwits sentiment....")
    trading_client.submit_order(order_data=market_sell)
    print("Short order for " + random_stonk[0] + " placed.")
elif (
    get_bear < get_bull and trading_client.get_asset(random_stonk[0]).shortable is False
):
